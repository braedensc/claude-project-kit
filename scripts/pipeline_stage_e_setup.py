...
def _read_dispatcher_facts_py(path):
    ...
    return (
        ...
        "(r.get('labelPrompts') or {}).values() "
        "for l in ((v if isinstance(v,list) else (v.get('labels') if isinstance(v,dict) "
        "and isinstance(v.get('labels'),list) else None)) or []) if isinstance(l,str)}),\n"
