...
CONFIG (--config FILE — the same file the poller reads; keys are shared)
  ...
  {"state_dir": "~/.stage-e/state",                beside the env file, under the daemon
   ...
   "required_checks": {"OWNER/REPO": ["Kit checks"]},   override of the base branch's
                                                   ...
                                                   applies to every repo). List EVERY
                                                   required context, including one a
                                                   session can never turn green — that
                                                   is `human_pending_checks`' job, and
                                                   excluding it here would hide it. The
                                                   override is the documented REMEDY when the
...
def blocked_signal(*, last_spent, delivered_bounces, blocked_row, head_sha, now,
    ...
    it, exactly as a partial exhaustion is completed — with one exception (KIT-152). A
    label recorded as LABEL_NOT_CONFIGURED counts as landed while `label_configured` is
    False: no id exists to apply, so retrying every pass would only repeat the problem.
    Configure an id and the row is incomplete again, so the next pass applies the label.
...
def decide(*, pr_open, is_draft, is_fork, ticket_terminal, ticket_state, trigger_ok,
    ...
    The three meet on a concluded PR. The settled conclusion is consulted first on both
...
def ledger_view(path, owner_repo, pr_number):
    """{'prior', 'last_spent', 'exhausted', 'concluded', 'notices', 'refreshes',
    'delivered_bounces', 'blocked'} for one PR. Only `outcome == "spent"` rows count
    toward the budget — those are appended BEFORE a send, so a failed send still spent
    its bounce (over-count, never under-count). A missing ledger reads as zero; an
    unreadable or corrupt one raises (read_ledger) rather than resetting the budget.
    ...
    does, and the newest row is the current state of it. `notices` is EVERY
    post-conclusion notice row, because its dedupe key is (head, kind) and not the pull
    request — the newest alone would say nothing about the other head or the other kind.
...
def selftest():
    ...
    # 5e. AFTER a conclusion. A conclusion is a HAND-OFF: the coding ticket is in the
    ...
    #     …and against the no-push signal (5d): a concluded PR is a person's, so a
    #     stall is never paged on it. With a live trigger it is the notice; with none
    #     it is the settled no-op — the person a stall would page already has it.
    check("a live trigger on a concluded PR NOTICES even when a stall is measured",
          decide(prior=1, **dict(base, blocked=stop,
                                 settled_conclusion={"basis": "clean", "moved": True}))["action"],
          "notice")
    check("…and inside the in-flight window too",
          decide(prior=1, **dict(held, blocked=stop,
                                 settled_conclusion={"basis": "clean", "moved": True}))["action"],
          "notice")

    # 5f. The notice body: what changed, where the ticket is, and no marker anything trusts.
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmp, contextlib.redirect_stderr(err):
            saved_add_label = globals()["linear_add_label"]

            def refusing_label(issue_id, label_id, cfg_):
                calls.append(("label", issue_id, label_id))
                raise BounceError("simulated: the tracker refused the label")
            globals()["linear_add_label"] = refusing_label
            try:
                sit_r = dict(sit_x, needs_human_label_id="lbl-nh")
                calls.clear()
                with contextlib.redirect_stdout(io.StringIO()):
                    rc_r = perform_exhaust(dict(sit_r), verdict_x, cfg, tmp, False)
                row_r = ledger_view(ledger_path(tmp), "o/r", 41)["exhausted"]
                check("KIT-152 a label the tracker REFUSES is retried: loud, recorded as not landed",
                      (rc_r, (row_r["announced"] or {}).get("label"),
                       any("simulated: the tracker refused" in p for p in (row_r.get("problems") or []))),
                      (EXIT_USAGE, False, True))
                check("KIT-152 …so the verdict still reads it as unannounced, and the next pass retries",
                      announcement_complete(row_r["announced"], True), False)
            finally:
                globals()["linear_add_label"] = saved_add_label
            calls.clear()
            with contextlib.redirect_stdout(io.StringIO()):
                rc_r2 = perform_exhaust(dict(sit_r, exhausted=row_r), verdict_x, cfg, tmp, False)
            check("KIT-152 …and the next pass applies it, and nothing else",
                  (rc_r2, kinds()), (EXIT_OK, ["label"]))
        world["delivery"] = delivery_ok
