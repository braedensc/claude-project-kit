...
def decide(*, pr_open, is_draft, is_fork, ticket_terminal, ticket_state, trigger_ok,
           ...
           cannot_evaluate="", conclusion=None, settled_conclusion=None, blocked=None):
    ...
    `blocked` (from `blocked_signal`) is the fourth thing that can sit where the quiet
    `skip` used to, and it is ranked LAST of them on purpose: it fires only where the
    driver would otherwise do nothing at all about this PR. Every other action already
    breaks the silence it exists to break — a bounce re-prompts the session, an exhaustion
    pages a person with the same label, a conclusion hands the work over, a CANNOT
    EVALUATE says so on the PR — and firing alongside one of those would page a person
    about a pipeline that is visibly moving. It replaces exactly two silences: the
    no-trigger skip (the live shape: the review that triggered the bounce now predates it,
    and only a push buys a re-review) and the in-flight skip (reachable whenever
    `in_flight_hours` is set longer than `blocked_after_seconds`)."""
...
def ledger_view(path, owner_repo, pr_number):
    """{'prior', 'last_spent', 'exhausted', 'concluded', 'delivered_bounces', 'blocked'}
    for one PR. Only `outcome == "spent"` rows count toward the budget — those are
    appended BEFORE a send, so a failed send still spent its bounce (over-count, never
    ...
    prompted at all."""
    prior, last_spent, exhausted, concluded, blocked = 0, None, None, None, None
    delivered = set()
    ...
    return {"prior": prior, "last_spent": last_spent, "exhausted": exhausted,
            "concluded": concluded, "delivered_bounces": delivered, "blocked": blocked}
...
def decision_for(sit, cfg):
    ...
    verdict = decide(pr_open=bool(meta.get("open")), is_draft=bool(meta.get("isDraft")),
                     ...
                     settled_conclusion=settled, blocked=blocked)
...
def selftest():
    ...
    # 5c. Where the no-push signal sits in that order: LAST of the things that can replace
