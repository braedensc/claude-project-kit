...
| **Stacking a branch or a PR** — `git checkout -b`/`switch -c`/`git branch <new> <start>`/`git worktree add -b` whose start point is not a base branch (**including the implicit start point**: branching again while HEAD is already on a feature branch), `git merge`/`git pull` of a feature ref, and `gh pr create\|edit --base <feature>` | Bash | **Every branch is cut from the base branch and merges back into it.** Squash-merging a base *rewrites* the base branch, so every PR stacked on it turns CONFLICTING at once — and GitHub runs **no checks at all** on a conflicted PR, so they read as "no checks reported", which looks like broken CI. A six-deep stack (#153←#154←#155←#156←#157←#158, 2026-09-20) cost five forced re-cascades and ended with four PRs closed. **Merging the base INTO a feature branch stays allowed** — `git merge origin/main` is the conflict-resolution move and what `pr-conflict-monitor.yml` asks a session to run, so the base set is an allow-list every arm consults and anything unparseable fails **open**; a false negative there would break the conflict loop, which is far worse than a stacked branch getting through. `git merge FETCH_HEAD` fails **closed** (the guard cannot see which ref was fetched) with a message saying to name the ref. Base branches = `main`/`master` **plus `github.defaultBranch` from the committed `delivery.json`** when a project configured the pipeline — read from the default branch, never the worktree, and **widening only**, so an unreadable config makes this stricter, never looser. Advisory like every Bash guard: the durable halves are the Stop hook's `baseRefName` check and the CI step `scripts/check_pr_base.py` |
...
| The open PR's **base is not the base branch** (`baseRefName` ∉ `main`/`master` + the configured `github.defaultBranch`) | **Every PR branches off the base and merges back into it.** Unlike the PreToolUse guard this reads GitHub's *own record*, so it does not care how the PR was created — a respelled command, the web UI, an automation. It deliberately fires on a **green** PR: a stacked PR looks fine right up until its base is squash-merged, at which point every descendant turns CONFLICTING and GitHub stops running checks on it entirely (six-deep stack, 2026-09-20 — five forced re-cascades). Asked **before** the rollup for the same reason `DIRTY` is: once it conflicts, "no checks" is a symptom the CI verdict cannot name. The message names `gh pr edit <n> --base <base>` and, unlike the two below, deliberately does **not** name `/fix-ci` — reading a job log fixes nothing here — and it says that retargeting fixes the *target*, not a branch that was also *cut* from the other branch. Fails open if `gh` does not return the field |
...
76 probes across the seven **Bash pattern** guards (secret paths, destructive ops,
...
that guard was added. Every probe recorded as getting through is then executed in a throwaway
