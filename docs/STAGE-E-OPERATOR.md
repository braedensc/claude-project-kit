...
| The Linear MCP tools stay in every **coding** session | Sessions may read Linear and comment across tickets. The brief says not to; the brief is not a boundary. Live test 7 measures it. Sessions can also move or archive a ticket they can reach, the heartbeat monitor's ticket included. Its comments then fail (no ticket yet; card `CK-9`). **Closed for the reviewer on 2026-09-16 (KIT-132):** the review entries fence every dispatcher-injected MCP server and every server the platform MCP configs add. That closure is an installer change, a run of the installer and a probe of a live reviewer — see *Changing the fence*. A coding session still needs the tracker to report its work. |
