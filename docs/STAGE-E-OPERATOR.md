...
- **The installer writes that entry for you**, with YOUR `gh` login rather than the
  ...
  It writes `human_pending_checks` at the same time, from `HUMAN_PENDING_CHECKS` in
  `stage-e.conf`, and reports what that bought — including any name that matches no
  required context on any reviewed repository, which buys nothing and is usually a typo.
  Because the installer rewrites both files from the conf, **a hand edit to either key is
  undone by the next run**: change `stage-e.conf` instead.
