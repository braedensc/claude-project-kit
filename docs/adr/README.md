...
| [Human-action notifier + reply relay](2026-09-06-human-action-notifier-and-reply-relay.md) | 2026-09-06 | **Proposed.** The events that need a person (blocked-session question, PR ready to merge, review declined, bounce budget spent, epic awaiting approval) are pushed to a dedicated one-person chat channel by a dispatcher-side job beside the Stage E poller — never by a session, so no allowlist widening and no credential in a session. Replies re-enter only as one comment in one session thread, sender-pinned and reply-to addressed, with no command grammar; approve/merge/label/state never transit the channel. Channel chosen on its reply API (outbound poll, no inbound URL): Telegram for the reference deployment. One-way first. Proposes the notifier as the `agent:blocked` writer under a pin-less dispatcher |
