...
>   A first snapshot takes the newest session whose `creator` is a person. With
>   `dispatcher_app_user_id` set, the session must also be the dispatcher's (`appUser`).
>   The tracker leaves `creator` empty for a session started by automation or an agent, so
>   a session cannot open a second one to re-take its snapshot. The branch hint named
>   above plays no part.
...
>   window, is unknown, because the edit may be inside the snapshot. A snapshot with no
>   criteria is still the basis. The review declines on it, and criteria added later make
>   the flag true; they never become the basis by falling through to the live ticket.
...
>   gives counts only: criteria added, criteria removed, and whether the out-of-scope list
>   changed. It says the review judges against the criteria as delegated, or declines for
>   a ticket delegated with none. It says that removing the delegation and delegating the
>   ticket again makes the new ones the basis. It quotes no ticket text, because it is
>   posted under the owner's key. That comment is the pass's only tracker write. It is
...
- The **dispatch-time snapshot** as a hard, unforgeable, pin-carried fact. Replaced by a
  ...
  *(Since 2026-09-16, KIT-131: tier 2 takes the snapshot on the first pass after a person
  delegates, with its lag recorded. Closer to the pin, and still not it (no ticket yet).)*
