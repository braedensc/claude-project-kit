...
| `agent:needs-human` | dispatcher | `maxBounces` or `totalAttempts` exhausted, or a `riskPaths` change. Terminal until a person acts. **Also: a bounce that was delivered and never answered with a push.** The Stage E bounce driver applies it when a `delivered` ledger row exists for the last bounce, the pull request's head is still the commit that bounce was delivered against, and more than `blocked_after_seconds` (default 21600) have passed — once per bounce, alongside one top-level comment on the coding ticket, spending no bounce and moving the ticket nowhere. Without it a session that stops mid-bounce is the pipeline's quietest state: the trigger is gone until a re-review, a re-review needs a push, and only a *spent* budget ever called a person. The condition reads the ledger, the head and the clock and never the session's reply, because a reply is a string the counted party writes. |
