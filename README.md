...
| `.github/workflows/pr-conflict-monitor.yml` + `scripts/pr_conflict.py` | The kit's own standing conflict watch — when an open PR goes `CONFLICTING` (a conflicted PR skips the required CI and can still look green), requests a fix — answered by the conflict waker, a supervised LaunchAgent that wakes a local session in its worktree (`scripts/pipeline_conflict_waker_setup.py`), or for a dispatcher's PR by the bounce driver in the session's own thread — and pages a person (the PR's author, or `PR_CONFLICT_PAGE_TO`) only when that stalls |
| `.github/workflows/pr-union-check.yml` + `scripts/check_pr_union.py` | Green alone, red together — runs the battery on the union of all open PRs and names the PR that broke it. Report-only |
...
| `docs/SESSION-BRIEF.md` | **Pipeline only**: the committed, generic brief every dispatched session and reviewer reads first — authority, the sandbox, the never-do list, what Stage E does to a PR, and the runbook for the unexpected |
| `docs/STAGE-E-OPERATOR.md` | **Pipeline only**: operator steps for Stage E (automated review + bounded fix re-prompt of dispatcher-opened PRs) — the Reviews team, the dispatcher's review entry, the two system daemons that run the poller and bounce driver as the dispatcher's role account, dry run, live tests |
